...
- [x] AC1: `FREEZE-MANIFEST.json` binds untreated source commit `6d65830`,
  its tree, alpha.6 release/runtime identity and 20 current runtime/evaluation
  file hashes plus the exact run-009 model/host/result identity. The freeze
  check reads baseline bytes from that Git commit, validates current approved
  requirements/audit/distribution identities and binds an authority commit
  whose 34 changed paths contain no treatment surface.
- [x] AC2: `CASE-AUTHORITY.json` contains 20 stable SAGR cases and 37 criteria,
  with all 20 approved categories represented exactly once, required and
  forbidden behaviours, three declared control pairs, three explicit state
  transitions and three ordered multi-turn sequences.
- [x] AC3: `PROVENANCE-REVIEW.md` records a case-by-case synthetic privacy
...
- [x] AC4: `build_goal_review_freeze.py --check` reproduces the derived
  inventory, validates exact IDs/categories, ordered turns, substantive text,
  control pairs, state assertions and frozen identities, and rejects treatment
  paths/runtime inclusion. Eight focused tests and the existing
  evaluation/privacy validators pass.
- [x] AC5: `CLAIM-PROOF-MATRIX.md` separates documented semantics,
...
| ---------- | ----------- | ---------- | -------- | -------- |
| . | `codex/strategic-alignment-goal-review` from `6d65830` | 179 unit tests; freeze, evaluation-build, evaluation-scope, privacy and diff checks pass | Local implementation only; no push, PR, merge, package, publication, activation or private-workspace mutation | `VALIDATION.md`, frozen artifacts and repository diff |
...
| 5 | Remediate adversarial QA | Add omitted phase-change, classification and milestone behaviours; strengthen pairing, state, identity and isolation checks; correct downstream lifecycle. | AC1-AC5 | Re-run the frozen checks without viewing treatment output. | Done | 4 | TASK-031 authority, builder/tests and workflow evidence only | No | bounded-return |
