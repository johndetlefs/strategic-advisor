...
- R4. Use condition-masked quality scoring: randomise presentation order, remove condition labels, precommit scorer model/version and prompt, prohibit quality scorers from guessing or receiving the apparent condition, run at least two fresh same-family scoring passes with deterministically swapped A/B labels, and apply a precommitted disagreement/adjudication rule. Run a separate structure-only leakage audit for stylistic identifiability; report substantive identifiability descriptively but do not treat it as failure. Retain raw outputs, mappings, quality scores, leakage-audit results, hard-gate verdicts, timestamps, configuration identity, and errors. Any quality score based on apparent condition is invalid.
...
- D5. Treat parent “blind scoring” as condition-masked quality scoring, disclose possible structural inference, prohibit condition guessing inside quality-scoring calls, and precommit two fresh same-family passes with deterministically swapped A/B labels plus adjudication. Run a separate structure-only leakage audit; substantive identifiability is descriptive rather than a failure. Same-family scoring passes are repeated evidence, not independent human validation.
...
- D7. Owner approved Option A on 2026-07-22 before any treatment, control, scorer, adjudicator, assertion-grader, trigger, or leakage-audit output was generated.
