...
As an individual user, I want a small explicit retention and authority policy,
so that my exact circumstances can inform strategy while durable storage and
external disclosure match the environment I chose.
...
Create the generic policy contract consumed by workspace tooling and canonical
runtime behaviour.

## Approach

Add one canonical context-policy reference, route workspace use through it, and
encode a simple machine-checkable policy block without expanding into an
identity or compliance system.

## Phases

1. Define the three modes and authority matrix.
2. Integrate policy into canonical workspace/runtime instructions.
3. Add deterministic policy fixtures and validation.

## Validation

- AC1: Review canonical policy and authority language.
- AC2: Run positive/negative mode validation fixtures.
- AC3: Run authority-matrix tests and aggregate validation.
