...
As a user, I want a compact but extensible personal-strategy workspace and safe
migration path, so that exact durable context has an explicit home without
flattening everything into one file or modifying my existing workspace.
...
- AC5: owner `Expand The Strategy Workspace And Migration Contract`; required evidence: Repeated generic scaffold identity and exact-value workspace fixtures.
- AC6: owner `Expand The Strategy Workspace And Migration Contract`; required evidence: Copy-based migration tests, overwrite refusal, extension validation, and negative leakage fixtures.
...
Replace the sterile exact-five-file scaffold with a deterministic personal
strategy contract and reversible migration.

## Approach

Extend templates and validator in place, keep standard-library-only tooling,
add an explicit `migrate` command, and use temporary synthetic fixtures for all
private-data-shaped tests.

## Phases

1. Expand templates and schema.
2. Support declared linked details and safe paths.
3. Implement copy-based migration and review output.
4. Run deterministic and package regression validation.

## Validation

- AC1-AC2: Focused workspace build/validate/link tests.
- AC3: Migration source-hash, destination, report, and overwrite tests.
- AC4: Full workspace, runtime-package, install-artifact, aggregate, and diff
  validation.
...
- Task: TASK-018
- Title: Expand The Strategy Workspace And Migration Contract
- Created: 2026-07-26
