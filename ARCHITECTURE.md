...
The current seven lenses are project/product, career, organisational influence, people leadership, business/venture, marketing/growth, and technical architecture. They reuse the core rather than redefining it. Technical architecture owns material system, repository, deployment, sharing, runtime/data/auth, build/adopt/migrate, coupling, operability, reversibility, and future-change-cost decisions; project/product retains ownership of intended outcome, product scope, experiment, and sequence. The conversational contract activates the core only for a material decision, including an architecture commitment expressed in ordinary language or newly exposed during implementation, then returns to the bounded task.

Before consequential scoring or commitment advice, the core locks only the material decision baseline: the exact artifact, revision, environment, state, scope, journey, and alternatives that could change the conclusion. A stale prototype, uninspected document, mismatched comparison, or demonstration state cannot silently stand in for the current decision object. This is not a mandatory response form; routine factual and implementation assistance remains direct when baseline identity cannot change the answer.
