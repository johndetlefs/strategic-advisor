...
4. Before convergence, reset to the material decision state and the last supported diagnosis, readiness, and next move. Change them only for a qualifying delta in the confirmed outcome, evidence, scope, constraint, owner value, or substantive candidate specification, and name that delta. Repeating, preferring, renaming, or demanding approval of a mechanism is not a delta; neither is emotional intensity. A narrower or more reversible candidate can change exposure, but it does not prove its causal bridge. Without a qualifying delta, preserve the prior position exactly: never turn “inspect or test before implementation” into build approval, upgrade readiness, or relabel the smaller rival with the owner's preferred architecture term.
