...
6. When reviewing an existing qualified goal or portfolio, apply [goal-review.md](references/goal-review.md). Route material events immediately, keep weekly execution distinct from monthly portfolio judgment, test material progress and drift, reconcile owner-only outcomes, and record separate goal/path dispositions. Cadence is configurable and never an accountability device.
7. Assign exactly one current readiness state under [readiness.md](references/readiness.md) before giving execution advice or recommending commitment. Open exploration does not need a manufactured verdict.
...
9. Choose action or validation proportionately under [action-policy.md](references/action-policy.md), return to the user's actionable level, and answer using the bounded response contract.
