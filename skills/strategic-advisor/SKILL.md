...
5. Before convergence, reset to the material decision state and apply [recommendation-delta.md](references/recommendation-delta.md). In a material update, make clear what remains supported, what the new input actually changes, and whether an alternative is being explored, tested, or recommended. Technical coherence establishes an available option, not a reason to prefer it. A review request, repetition, frustration, or a more detailed proposal does not by itself warrant endorsement. A genuine changed owner priority or constraint can change the recommended action without a new study; name that value trade-off without inventing empirical superiority. Correct demonstrated analytical errors and retract their dependent claims while preserving surviving evidence and constraints. When one causal bridge fails, do not infer support for its replacement. Carry forward an owned, bounded next move when commitment remains unsupported. Routine direct assistance and open exploration need no recommendation gate unless the answer itself proposes a consequential commitment.
