...
## Condition-masked scoring

- Do not provide condition labels, skill identity, expected outcomes, or case assertions to scorers.
- Before scoring, require each scorer to guess `A`, `B`, or `unclear` as the likely skilled condition, give confidence from 1 to 5, and classify the basis as `structure`, `substance`, `mixed`, or `none`. Record the guess before dimension or gate scoring.
- Use two fresh scoring contexts with the precommitted scorer model, version, configuration, and prompt for every case/draw pair. A scoring context contains one pair and one pass only; never reuse it across cases, draws, or passes, and never provide prior scores, guesses, assertions, outputs from other pairs, or the A/B mapping.
- Generation also uses a fresh context for every case, draw, and condition. The second draw is not a continuation, retry, or revision of the first. A parser retry uses a new context and does not count as an independent draw or scoring pass.
- Adjudication uses a fresh context for one case/draw and one disputed subject. It receives only that dispute and never receives condition labels, mappings, assertions, unrelated scores, or another case.
- A scorer output whose applicability does not exactly match the frozen `not_applicable_dimensions` list is schema-invalid and receives the single parser retry; applicability is never adjudicated. Adjudicate when an applicable dimension score for either response differs by 2 or more, or any hard-gate verdict differs. The adjudicator must return final numeric scores for both A and B for a dimension or final booleans for both A and B for a hard gate.
- For applicable, non-adjudicated dimensions, use the arithmetic mean of the two integer scores separately for A and B. For adjudicated dimensions, use the adjudicator's final score. A hard gate fails when both scorers fail it or adjudication confirms it after disagreement; a missing or invalid verdict is an evaluation error, not a pass.

### Parser contract and retry limit

The scorer, adjudicator, and case-assertion grader must emit one JSON object matching their frozen prompt schema exactly: no prose, Markdown fence, comments, duplicate keys, omitted keys, unexpected keys, stringified numbers or booleans, or non-finite values. Validate IDs, enums, complete dimension and hard-gate key sets, score/null applicability consistency, evidence fields, and derived booleans before accepting an artifact. Emit only these stable parser diagnostic codes on rejection: `E_JSON_PARSE`, `E_SCHEMA_KEYS`, `E_SCHEMA_TYPE`, `E_SCHEMA_ENUM`, `E_SCHEMA_ID`, `E_SCHEMA_COMPLETENESS`, `E_APPLICABILITY_MISMATCH`, `E_DERIVED_VALUE`, `E_EVIDENCE_EMPTY`, or `E_IDENTITY_BIAS`; multiple codes may apply.

On schema or JSON failure, retain the invalid raw artifact and machine diagnostic, then allow exactly one retry in a fresh context with the same frozen inputs plus only the parser diagnostic codes. Do not include the invalid output, condition mapping, expected answer, or repair suggestion. If the retry fails, mark that scoring or grading pass errored and fail the iteration under [AGGREGATION.md](AGGREGATION.md) section 4. Never hand-edit, coerce, fill, or infer a missing numeric or boolean outcome; retain the pair and do not launch further retries inside the iteration.

### Condition-guess leakage and claim limits

Report for every scoring pass: guess, confidence, basis type, correctness after unmasking, and whether score evidence refers only to response quality. Aggregate total guess accuracy with `unclear` counted as not correct; also report determinate A/B accuracy, unclear rate, and the count and accuracy of guesses whose basis is `structure` or `mixed`.

Never call the procedure blind; it is condition-masked. Any determinate structural guess must be disclosed. If a scorer's score evidence rewards or punishes an output because it appears skilled, invalidate that scoring pass and use the single parser/contract retry. If the retry repeats the defect, retain an errored pass.

Flag **systematic condition leakage** when there are at least 20 determinate A/B guesses of any self-reported basis and at least 70% are correct. Report structure/mixed guesses separately, but do not let the scorer's own basis classification decide whether leakage counts. The flag does not permit post-hoc score adjustment and does not erase observed output differences. It fails the automated release gate for that iteration because the same judge may be rewarding a recognisable response contract. A separately preregistered independent-human or meaningfully different-judge confirmation may be reported as new evidence, but it cannot rewrite this iteration's failed gate. Even below the flag threshold, do not claim that structural inference was impossible.
...
9. Systematic condition leakage is not flagged. If it is flagged, the automated release gate fails even when score thresholds pass.
